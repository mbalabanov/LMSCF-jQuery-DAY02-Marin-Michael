...
function addItemToCart() {
	let item = this.id;
	productdata.jewelry[item].amount_cart = '1';
	...
	if (jQuery.inArray(item, ordersClick) == '-1') {
		...
		$('.addedButton').click(function() { $('#cartabsolute').hide(); });
	}
}
...
function generateOrderTable() {
	...
	for (let i in ordersArray) {
		let currentAmountPrice = ordersArray[i].amount_cart * ordersArray[i].price;
				...
				<table id="ordertable">
					<tr>
						...
						<td class="namecolumn">
							...
							<div class="productprice"><strong>$${ordersArray[i].price} pp</strong> (${ordersArray[i].amount_stock} in stock)</div>
						</td>
						<td class="amountcolumn"><input type="number" maxlength="2" min="0" max="${ordersArray[i].amount_stock}" value="${ordersArray[i].amount_cart}" class="itemAmountField" id="ia${ordersArray[i].id}"></td>
	...
	$('.itemAmountField').change(function() {
		...
		productdata.jewelry[itemIndex].amount_cart = tempValue;
		generateOrderTable();
	});	
};
